...
interface ForwardConnectionRequest {
    url: string;
    accessToken?: string;
...
export default defineEventHandler(async (event) => {
    ...
    const body = await readBody<ForwardConnectionRequest>(event);

    if (!body.url) {
        throw createError({
            statusCode: 400,
            statusMessage: 'Missing URL for forward connection'
        });
    }
    ...
    try {
        ...
        const success = await onebotForwardClient.createConnection(connectionConfig);
