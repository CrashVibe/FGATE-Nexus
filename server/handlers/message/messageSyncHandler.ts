...
export class MessageSyncHandler {
    ...
    private async sendOnebotGroupMessage(
        ...
        try {
            ...
            const messageData = {
                ...
                echo: `message_sync_${Date.now()}`
            };

            const messageJson = JSON.stringify(messageData);

            // 尝试反向连接
            const reverseConnection = onebotManager.getBotConnection(botId);
            if (reverseConnection && reverseConnection.peer) {
                reverseConnection.peer.send(messageJson);
                console.log(`[MessageSync] [REVERSE] 发送消息到群 ${groupId}: ${message}`);
                return true;
            }
            ...
            try {
                ...
                if (success) {
                    console.log(`[MessageSync] [FORWARD] 发送消息到群 ${groupId}: ${message}`);
                    return true;
                }
            } catch (forwardError) {
            ...
            console.warn(`[MessageSync] 机器人 ${botId} 无可用连接发送消息到群 ${groupId}`);
            return false;
        } catch (error) {
            console.error(`[MessageSync] 发送群消息失败 (Bot ${botId}, Group ${groupId}):`, error);
            return false;
        }
    }
    ...
    private async getOnebotConnectionStatus(adapterId: number | null): Promise<ConnectionStatus['onebot']> {
        ...
        try {
            ...
            if (isConnected) {
                const connection = onebotManager.getBotConnection(adapter.botId || 0);
                lastSeen = connection?.lastHeartbeat?.toISOString() || new Date().toISOString();
            }
