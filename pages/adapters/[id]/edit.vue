          ...
          <template v-if="formData.adapter_type === 'onebot'">
            ...
            <div class="form-section">
              ...
              <div class="form-grid">
                <!-- 只有反向连接需要 botId -->
                <n-form-item
                  v-if="formData.config.onebot.connectionType === 'reverse'"
                  label="机器人 ID"
                  path="config.onebot.botId"
                >
                  <n-input-number
            ...
            <div class="form-section">
              ...
              <template v-if="formData.config.onebot.connectionType === 'forward'">
                <n-form-item label="WebSocket URL" path="config.onebot.forwardUrl">
                  <n-input
                    v-model:value="formData.config.onebot.forwardUrl"
                    placeholder="ws://localhost:5700"
                    clearable
                  />
                </n-form-item>
                  ...
                  <div class="connection-test">
                    <n-space>
                      <n-button
                        ...
                        :disabled="!formData.config.onebot.forwardUrl"
...
import { getOnebotRules } from '~/utils/adapters/rules';
...
const canSubmit = computed(() => {
  if (formData.value.adapter_type === 'onebot') {
    ...
    if (connectionType === 'reverse') {
      return config.botId && config.responseTimeout;
    } else {
      return config.responseTimeout; // 正向连接只需要 responseTimeout
    }
  }
  return false;
});
...
const currentRules = computed(() => {
  ...
  if (formData.value.adapter_type === 'onebot') {
    const connectionType = formData.value.config.onebot.connectionType || 'reverse';
    const rules = getOnebotRules(connectionType);

    return {
      ...baseRules,
      'config.onebot.botId': rules.botId,
      'config.onebot.responseTimeout': rules.responseTimeout
    };
  }
  return baseRules;
});
...
function onConnectionTypeChange(type: 'reverse' | 'forward') {
  if (type === 'forward' && !formData.value.config.onebot.forwardUrl) {
    formData.value.config.onebot.forwardUrl = 'ws://localhost:5700';
  }
}
...
async function handleSubmit() {
  ...
  try {
    ...
    const payload: AdapterPayload = { adapter_type };

    if (adapter_type === 'onebot') {
      const data = config.onebot;

      // 验证必填字段
      const connectionType = data.connectionType || 'reverse';
      if (connectionType === 'reverse' && !data.botId) {
        message.error('反向连接模式下，请输入机器人 ID');
        return;
      }

      payload.config = {
        botId: connectionType === 'reverse' ? data.botId : null, // 正向连接时 botId 为 null
        accessToken: data.accessToken?.trim() || null,
        responseTimeout: data.responseTimeout,
        enabled: data.enabled,
        connectionType: data.connectionType,
        forwardUrl: data.connectionType === 'forward' ? data.forwardUrl : null,
        autoReconnect: data.connectionType === 'forward' ? (data.autoReconnect ?? true) : undefined
      };
    }

    if (isEdit.value) {
      useRequest(adapterApi.updateAdapter(adapterId.value!, payload))
        .onSuccess(({ data }) => {
          if (data.success) {
...
async function testForwardConnection() {
  if (!adapterId.value || !formData.value.config.onebot.forwardUrl) return;

  testConnecting.value = true;

  try {
    const { data } = await useFetch(`/api/adapters/${adapterId.value}/forward`, {
      method: 'POST',
      body: {
        url: formData.value.config.onebot.forwardUrl,
        accessToken: formData.value.config.onebot.accessToken || undefined,
        autoReconnect: formData.value.config.onebot.autoReconnect
      }
    });

    const response = data.value as { success: boolean; message: string };
...
async function refreshConnectionStatus() {
  ...
  try {
    ...
    const response = data.value as {
      success: boolean;
      data: { botId: number; connected: boolean; readyState: number | null };
    };
